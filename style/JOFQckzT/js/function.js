...
function reportErr(id){openWin("https://www.xigua911.com/err.php?id="+id,400,220,350,250,0)}

function AddFav(vid,uid)
{
	ajax.get( 
		"https://www.xigua911.com/include/ajax.php?action=addfav&id="+vid+"&uid="+uid,
		function(obj) {
			if (obj.responseText=="err"){
				alert("请先登录!");
				location.href="https://www.xigua911.com/login.php";
			}else{
				alert("收藏成功!");
			}
		}
...
function viewComment(id,page){
	var url;
	if (page.length==0){url=id;}else{url="https://www.xigua911.com/comment.php?id="+id+"&page="+page;}
	ajax.get( 
		url,
		function(obj) {
			if (obj.responseText=="err"){
...
function diggVideo(id,div){
	ajax.get(
		"https://www.xigua911.com/include/ajax.php?id="+id+"&action=digg",
		function (obj){
...
function treadVideo(id,div){
	ajax.get(
		"https://www.xigua911.com/include/ajax.php?id="+id+"&action=tread",
		function (obj){
			var returnValue=Number(obj.responseText)
...
function diggNews(id,div){
	ajax.get("https://www.xigua911.com/include/ajax.php?id="+id+"&action=diggnews",function (obj){
			var returnValue=Number(obj.responseText)
			if (!isNaN(returnValue)){set(document.getElementById(div),returnValue);alert('(*^__^*) 嘻嘻……，顶得我真舒服！');}else if(obj.responseText=="err"){alert('顶失败')}else if(obj.responseText=="havescore"){alert('(*^__^*) 嘻嘻…… 这么热心啊，您已经顶过了！')}	
		}
	);
...
function treadNews(id,div){
	ajax.get("https://www.xigua911.com/include/ajax.php?id="+id+"&action=treadnews",function (obj){
			var returnValue=Number(obj.responseText)
			if(!isNaN(returnValue)){set(document.getElementById(div),returnValue);alert('小样儿，居然敢踩我！');}else if(obj.responseText=="err"){alert('踩失败')}	else if(obj.responseText=="havescore"){alert('我晕，您已经踩过了，想踩死我啊！')}	
		}
	);	
...
function getVideoHit(vid){
	ajax.get(
		"https://www.xigua911.com/include/ajax.php?action=hit&id="+vid,
		function (obj){
...
{
	ajax.get( 
		"https://www.xigua911.com/include/ajax.php?action=member",
		function (obj){
			var result=obj.responseText;
...
function getNewsHit(nid){
	ajax.get(
		"https://www.xigua911.com/include/ajax.php?action=hitnews&id="+nid,
		function (obj){
			var result=obj.responseText
...
function markscore0(vd,d,t,s,l,ac){
	var alt=['很差','较差','还行','推荐','力荐'],url=ac=='news' ? ["https://www.xigua911.com/include/ajax.php?id="+vd+"&action=newsscore","https://www.xigua911.com/include/ajax.php?id="+vd+"&action=scorenews&score="] : ["https://www.xigua911.com/include/ajax.php?id="+vd+"&action=videoscore","https://www.xigua911.com/include/ajax.php?id="+vd+"&action=score&score="],
...
function markscore1(vd,d,t,s,l,ac){
	var alt=['很差','较差','还行','推荐','力荐'],src=['/'+sitePath+'pic/star0.gif','/'+sitePath+'pic/star1.gif'],url=ac=='news' ? ["https://www.xigua911.com/include/ajax.php?id="+vd+"&action=newsscore","https://www.xigua911.com/include/ajax.php?id="+vd+"&action=scorenews&score="] : ["https://www.xigua911.com/include/ajax.php?id="+vd+"&action=videoscore","https://www.xigua911.com/include/ajax.php?id="+vd+"&action=score&score="],
...
function markNews2(vid,style,len){
	...
	ajax.get(
		"https://www.xigua911.com/include/ajax.php?action=npingfen&id="+vid,
		function (obj){
			var result=obj.responseText;
			result=result.split(",");
			num=result[0];
...
function markVideo2(vid,style,len){
	...
	ajax.get(
		"https://www.xigua911.com/include/ajax.php?action=vpingfen&id="+vid,
		function (obj){
			var result=obj.responseText;
			result=result.split(",");
			num=result[0];
...
function addRemoteFavor(){
	ajax.get(
		"https://www.xigua911.com/include/ajax.php?action=favorAjax&id="+play_vid+"&faction=add",
