...
function reportErr(id){openWin("/"+sitePath+"err.php?id="+id,400,1000,1000,1000,0)}
...
function leaveWord(){
	...
	ajax.postf(
		...
		function(obj){if(obj.responseText=="ok"){viewLeaveWordList(1);alert('留言成功，多谢支持！');document.getElementById("m_content").value='';}else if(obj.responseText=="haveleave"){alert('手速也太快了吧，歇会儿再来留言吧！');}else{alert('发生错误');}}
	);
}
